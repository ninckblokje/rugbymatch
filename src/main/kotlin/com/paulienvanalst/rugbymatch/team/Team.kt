...
data class Team (val players: List<Player>, val name: TeamName) {
    ...
    val hasAnySubstitutes : Boolean
        get() = players.any { player -> !player.isStarting }
    ...
    fun captainBackNumber(): Int? {
        //return players.first { it.backNumber == 7 }.backNumber
        return players.find { it.backNumber == 7 }!!.backNumber
    }
    ...
    fun replacingCaptainBackNumber(): Int? {
        return players.find { player -> player.isStarting }?.backNumber
    }
